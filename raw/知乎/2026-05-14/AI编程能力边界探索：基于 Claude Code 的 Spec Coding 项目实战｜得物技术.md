...
published:
created: 2026-05-14
description: "一、前言10 天，2.5 万行代码，提效 36%。 基于 Claude Code 的 Spec Coding（规格驱动编码） 深度实战。通过 2,754 次工具调用，我们不仅完成了从 0 到 1 的前端项目搭建，更在“约束+示范+视觉”的三层规范体系…"
tags:
  - "clippings"
---
222 人赞同了该文章
...
10 天，2.5 万行代码，提效 36%。 基于 Claude Code 的 [Spec Coding](https://zhida.zhihu.com/search?content_id=271340437&content_type=Article&match_order=1&q=Spec+Coding&zhida_source=entity) （规格驱动编码） 深度实战。通过 2,754 次工具调用，我们不仅完成了从 0 到 1 的前端项目搭建，更在“约束+示范+视觉”的三层规范体系下，摸清了 AI 编程的真实能力边界。本文将复盘这场实战，拆解如何用结构化工作流消除 AI 的不确定性，重构开发者的核心竞争力。

![](https://pic3.zhimg.com/v2-1295a999983bba042b2195ccccebfb04_1440w.jpg)

## 二、Spec Coding

### 什么是 Spec Coding 工作流

众所周知，Spec Coding（规格驱动编码）的核心思想是：在写代码之前，先写规格文档。通过 [openspec](https://zhida.zhihu.com/search?content_id=271340437&content_type=Article&match_order=1&q=openspec&zhida_source=entity) 工具，每个功能变更都经历以下阶段：

![](https://pic2.zhimg.com/v2-0ab7da2f35dd0b5c2f67e6cb9b65ac61_1440w.jpg)
...
![](https://pic3.zhimg.com/v2-30268dee5d27c8fe47815c6701845764_1440w.jpg)

## 四、数据概览

在这次使用Claude Code 做 Spec Coding的从0到1项目探索中，我们积累了一份完整的原始数据，以下所有数字均来自Claude Code对 109 个.jsonl 会话文件的整体数据统计：

![](https://pica.zhimg.com/v2-fb1a68185a4500330f8eaed2f3c3d654_1440w.jpg)

![](https://picx.zhimg.com/v2-a60b9d92658846bdb2eed927c0deb9d9_1440w.jpg)

2,754 次工具调用的分布揭示了 AI 的"工作方式"， AI 自主完成的 738 次文件读取、550 次代码编辑、662 次终端命令执行，以及 208 次任务进度标记——几乎覆盖了一个研发日常工作的全部动作类型。

## 五、开发时间线：10 天的演进过程

![](https://picx.zhimg.com/v2-259c5d6876d2f7fed8929719db2e2da5_1440w.jpg)

### 阶段一：设计阶段

![](https://pic2.zhimg.com/v2-a5a084409876023fd4065930134dd383_1440w.jpg)
...
![](https://pic3.zhimg.com/v2-b38b9e6900971d1d4f868ac4b8b5f7c8_1440w.jpg)
...
![](https://pic2.zhimg.com/v2-b5a422c2f04629e11bbdf791adbad3e9_1440w.jpg)
...
![](https://pic3.zhimg.com/v2-1d80e56d55b1157d123d67825992de1e_1440w.jpg)
...
![](https://pic3.zhimg.com/v2-cdb4af657f5dcb42bfb48b37fa475ab8_1440w.jpg)

**让 AI 扮演产品经理：**

在 Rules 中植入「首席产品专家」Persona 提示词，将 AI 从工程师的「急于执行」模式切换为产品经理的「先想清楚」模式，与 AI 聊清楚我们想干什么。

![](https://pic2.zhimg.com/v2-3e83b8b2a4196a700fb2aa1a55523869_1440w.jpg)

**让 AI 扮演 UI 设计师：**

在 Rules 中定义设计规范，通过对话式生成逐页产出高保真 HTML 文件，而不是源码：

![](https://pica.zhimg.com/v2-8fc127cdbef5c0cdc06e5a6c034da35c_1440w.jpg)

**让 AI 生成研发可读的 PRD：**

基于产品经理角色，将 HTML 设计稿作为上下文，最后生成精确到组件行为级别的 PRD：

![](https://pic2.zhimg.com/v2-0b2c1d55c10c551be101fdcc540fc919_1440w.jpg)

### 案例二：SDD 驱动前端功能研发

在已有系统上增量交付一个完整功能模块，SDD 如何保证「增量」功能快速开发，并系统性提升前后端联调效率。比如其中有个SSD需求开发「定时任务管理」完整模块，并且对接 6 个后端接口。这是 SDD 工作流第一次被完整运用于新功能模块开发，也是验证「SDD + [MCP](https://zhida.zhihu.com/search?content_id=271340437&content_type=Article&match_order=1&q=MCP&zhida_source=entity) 」前后端联调提效的关键场景。

![](https://pic4.zhimg.com/v2-c0867bcf1953645e3799d7b52b413c4d_1440w.jpg)
...
![](https://pic2.zhimg.com/v2-f690af2168c624d18558567d1b52fbed_1440w.jpg)
...
![](https://picx.zhimg.com/v2-9782d7f7b7163a29b663dc21dbd17549_1440w.jpg)

其中有一天遇到一个测试环境构建失败的问题，结果过程约 4 小时，7 个会话、15+ 次方案尝试、59 条指令。整个项目单日指令最多的一天，也是 AI 独立解决能力最受限的一天。

![](https://pic1.zhimg.com/v2-0809267f9ea0960dbc187fe7655818b2_1440w.jpg)

![](https://pic1.zhimg.com/v2-c05a57748ec212c829ff7a6945b98604_1440w.jpg)
...
- PRISMA\_ENGINES\_MIRROR= [registry.npmmirror.com/](https://link.zhihu.com/?target=https%3A//registry.npmmirror.com/-/binary/prisma) 。
...
![](https://pic2.zhimg.com/v2-a82d39ef35cd44d0dbe5d86fd45d06db_1440w.jpg)
...
![](https://pica.zhimg.com/v2-bbd675657b47cf84d148863bb2dd6fc0_1440w.jpg)

![](https://picx.zhimg.com/v2-c709d2cb453097fb0f8cd9d96d22bdc9_1440w.jpg)
...
![](https://pic1.zhimg.com/v2-5606007efcb3c0d16fcbade3ca3ee374_1440w.jpg)
...
![](https://pic2.zhimg.com/v2-324a6bdbf87af48050127569b1b216e5_1440w.jpg)
...
![](https://pic3.zhimg.com/v2-c99075e208d97b87f2a70442c9aa42a0_1440w.jpg)
...
![](https://pic4.zhimg.com/v2-a7fa817b405120543a27b1a28ed147a5_1440w.jpg)
...
![](https://pic3.zhimg.com/v2-48095d0117449139bac3bbde90899d5e_1440w.jpg)
...
![](https://pic4.zhimg.com/v2-1c1f867d10147b096678e0f4c7343311_1440w.jpg)

### 值得期待的方向

基于本项目的数据和经验，后续在以下方向可作深入探索：

![](https://pic1.zhimg.com/v2-48bb82fddb478e55221f3d5b58a58bd8_1440w.jpg)
