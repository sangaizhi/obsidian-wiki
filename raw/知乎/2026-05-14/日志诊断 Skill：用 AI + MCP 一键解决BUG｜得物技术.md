---
title: "日志诊断 Skill：用 AI + MCP 一键解决BUG｜得物技术"
source: "https://zhuanlan.zhihu.com/p/2022738059486143021?share_code=dkNDgR12wDtA&utm_psn=2037598133916545072"
author:
  - "[[SmartCode得物技术]]"
published:
created: 2026-05-14
description: "一、概述做后端开发，调 BUG 有一个让人头疼的固定流程：打开日志平台，输入 traceId 或关键词，搜日志；从几十上百条日志里，找到关键的那几条；把日志里的类名、方法名复制出来，去 IDE 里找对应代码；结合代码…"
tags:
  - "clippings"
---
65 人赞同了该文章
...
做后端开发，调 BUG 有一个让人头疼的固定流程：打开 [日志平台](https://zhida.zhihu.com/search?content_id=272371711&content_type=Article&match_order=1&q=%E6%97%A5%E5%BF%97%E5%B9%B3%E5%8F%B0&zhida_source=entity) ，输入 traceId 或关键词，搜日志；从几十上百条日志里，找到关键的那几条；把日志里的类名、方法名复制出来，去 IDE 里找对应代码；结合代码逻辑，判断哪里出了问题；如果一次找不准，回去再搜日志，再翻代码……

这个过程相对固定，但非常耗时间。每次 BUG 定位，光在日志平台和 IDE 之间来回切换，就能消耗掉大半的时间。

最开始在去年 Q3 想到这个问题的时候，脑子里浮现的第一个方案是：用 Cursor + [MCP](https://zhida.zhihu.com/search?content_id=272371711&content_type=Article&match_order=1&q=MCP&zhida_source=entity) ，把日志平台接进来，再挂一个代码知识库，让 AI 帮我查日志。但这个方案有缺陷 —— 日志查询是「动态的」，它依赖环境、应用、时间范围，没办法静态预置。此外，这样处理没有办法做到比较丝滑地读代码、改代码。

后来开始用 [Claude Code](https://zhida.zhihu.com/search?content_id=272371711&content_type=Article&match_order=1&q=Claude+Code&zhida_source=entity) ，接触到了 Skill 的概念：可以在项目里定义一套自定义命令，描述 AI 应该怎么执行这个命令的每个步骤，于是整个思路变得清晰了。
...
MCP 本质上是一种标准化的「工具调用协议」，Claude Code 通过 SSE（ [Server-Sent Events](https://zhida.zhihu.com/search?content_id=272371711&content_type=Article&match_order=1&q=Server-Sent+Events&zhida_source=entity) ）长连接与 MCP Server 通信，实时获取日志数据。

## MCP 环境对照

![](https://pic4.zhimg.com/v2-f42a4c5babe7beb7d24837fe46cc7ab5_1440w.jpg)

## 核心 MCP 工具

![](https://pic2.zhimg.com/v2-1135d3803032f1d1076efdcdfa221271_1440w.jpg)
...
![](https://pica.zhimg.com/v2-0c6e80d3ae620d9156c557176244ce04_1440w.jpg)
...
![](https://pic1.zhimg.com/v2-f5106e1e206495dc704bbf29787f792c_1440w.jpg)
...
AI 发现：其他字段都处理了 IS NULL 和 = ''（空字符串代表 “不限制”）两种情况，唯独 customer\_tag 只判断了 IS NULL，遗漏了空字符串 '' 的情况。
...
DB 中现有的数据，customer\_tag 字段都存的是空字符串（未配置），按业务语义本应匹配所有请求，却因为这个遗漏被全部过滤掉了。
...
![](https://pic4.zhimg.com/v2-a7afc6902a84ff7321fa7214e6955689_1440w.jpg)

这个 BUG 的隐蔽性在于：SQL 语法正确，逻辑上也「看起来」没问题——只有对比了其他字段的写法，才能发现 customer\_tag 独自遗漏了空字符串的处理。这类细节差异，人工排查很容易忽略，AI 反而很擅长。
...
发布于 2026-04-02 10:41・上海[阿里超强大模型，让问答不止于表面](https://www.qianwen.com/?ch=webtongyi@gp_zhztzhihugpwebty01_normal2_35&cb=https%3A%2F%2Fsugar.zhihu.com%2Fplutus_adreaper_callback%3Fsi%3Da39cacc6-ec45-4d8d-abce-631d74ddcf94%26os%3D3%26zid%3D1629%26zaid%3D3737725%26zcid%3D3725481%26cid%3D3725481%26event%3D__EVENTTYPE__%26value%3D__EVENTVALUE__%26ts%3D__TIMESTAMP__%26cts%3D__TS__%26mh%3D10d63d9a187b30b46c8b86b8e54b7044%26adv%3D755269%26ocg%3D1%26cp%3D200%26ocs%3D1%26aic%3D0%26atp%3D0%26ct%3D2%26ed%3DGiBNJgVzfCMmUW9XFyEvRA8xBGxJICwkOhh0FlwxKw1ZY0gnWzUoISkYZUFAPSBRDHgPbEkuNjNzFXQSRDkoDVpyD3kLe2N4fhdgXwRmegZNKlcvUSdncGhJJ1oFYHkIWn0Bc5cN5MuWEoL5&ua=Mozilla%2F5.0+%28Windows+NT+10.0%3B+Win64%3B+x64%29+AppleWebKit%2F537.36+%28KHTML%2C+like+Gecko%29+Chrome%2F148.0.0.0+Safari%2F537.36&os=3&campaignId=2736418&adgroupId=3737725&creativeId=3725481&accountId=755269&ip=27.44.140.141&ipv6=27.44.140.141&from=zhihu&spu=biz%3D0%26ci%3D3725481%26si%3D2ad704cc-b90b-4e46-9b76-ad38d87091e8%26ts%3D1778771878%26zid%3D1629)

[

依托阿里超强大模型，千问不仅能答“是什么”，更能解释“为什么”和“怎么做”。深度推理能力，助你穿透问题本...

](https://www.qianwen.com/?ch=webtongyi@gp_zhztzhihugpwebty01_normal2_35&cb=https%3A%2F%2Fsugar.zhihu.com%2Fplutus_adreaper_callback%3Fsi%3Da39cacc6-ec45-4d8d-abce-631d74ddcf94%26os%3D3%26zid%3D1629%26zaid%3D3737725%26zcid%3D3725481%26cid%3D3725481%26event%3D__EVENTTYPE__%26value%3D__EVENTVALUE__%26ts%3D__TIMESTAMP__%26cts%3D__TS__%26mh%3D10d63d9a187b30b46c8b86b8e54b7044%26adv%3D755269%26ocg%3D1%26cp%3D200%26ocs%3D1%26aic%3D0%26atp%3D0%26ct%3D2%26ed%3DGiBNJgVzfCMmUW9XFyEvRA8xBGxJICwkOhh0FlwxKw1ZY0gnWzUoISkYZUFAPSBRDHgPbEkuNjNzFXQSRDkoDVpyD3kLe2N4fhdgXwRmegZNKlcvUSdncGhJJ1oFYHkIWn0Bc5cN5MuWEoL5&ua=Mozilla%2F5.0+%28Windows+NT+10.0%3B+Win64%3B+x64%29+AppleWebKit%2F537.36+%28KHTML%2C+like+Gecko%29+Chrome%2F148.0.0.0+Safari%2F537.36&os=3&campaignId=2736418&adgroupId=3737725&creativeId=3725481&accountId=755269&ip=27.44.140.141&ipv6=27.44.140.141&from=zhihu&spu=biz%3D0%26ci%3D3725481%26si%3D2ad704cc-b90b-4e46-9b76-ad38d87091e8%26ts%3D1778771878%26zid%3D1629)