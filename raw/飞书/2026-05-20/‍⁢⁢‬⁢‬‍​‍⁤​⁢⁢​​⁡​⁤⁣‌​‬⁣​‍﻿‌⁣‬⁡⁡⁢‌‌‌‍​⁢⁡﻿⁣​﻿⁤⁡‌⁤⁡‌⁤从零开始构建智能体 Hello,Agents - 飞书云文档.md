---
title: "‍⁢⁢‬⁢‬‍​‍⁤​⁢⁢​​⁡​⁤⁣‌​‬⁣​‍﻿‌⁣‬⁡⁡⁢‌‌‌‍​⁢⁡﻿⁣​﻿⁤⁡‌⁤⁡‌⁤从零开始构建智能体 [Hello,Agents] - 飞书云文档"
source: "https://waytoagi.feishu.cn/wiki/M0WgwwFc0iSkknknxzKcN3i6nLb"
author:
published:
created: 2026-05-20
description:
...
  智能体是一个飞速发展且极度依赖实践的领域。为了获得最佳的学习效果，我们在项目的`code`文件夹内提供了配套的全部代码，强烈建议你将理论与实践相结合。请务必亲手运行、调试甚至修改项目里提供的每一份代码。欢迎你随时关注 Datawhale 以及其他 Agent 相关社区，当遇到问题时，你可以随时在本项目的 issue 区提问。
...
![](https://waytoagi.feishu.cn/space/api/box/stream/download/asynccode/?code=NDA5N2U0MTk0OGQ1NzI0ZmQ4YWM1ZjhkY2RhOWQ2M2VfdWlRTkp1QVNpdlZNaTNpckhXVWdIOER6T2pnUWV1bVJfVG9rZW46SUcyc2JmSWd6b055Qzl4cGtYUGNmb1JXbnVjXzE3NzkyODY2MDc6MTc3OTI5MDIwN19WNA)
...
![](https://waytoagi.feishu.cn/space/api/box/stream/download/asynccode/?code=YTYyYTUwYzE2N2MzODQzYjI5ZjE1MDlhMjM0NWU0ZjFfNGI3VE14VVRTUHJQN2pQRDBwYzlrazdUNGR6Vkh5WThfVG9rZW46VFpoRGJJS0Exb3NHSUx4bHk3T2NGMHQ4bm5nXzE3NzkyODY2MDc6MTc3OTI5MDIwN19WNA)
...
![](https://waytoagi.feishu.cn/space/api/box/stream/download/asynccode/?code=OTA2MDJjNzkzZDg4YTNjNmY2N2IyMjFhOTc2NWE0MWJfZ1VacWZPZHNGcndTU0lONk9nOEkxdTVsblpjVTZQZ1VfVG9rZW46V2xmUGJEVHg5b0RpQkp4Y3dXNGNKWDhYbkpnXzE3NzkyODY2NTQ6MTc3OTI5MDI1NF9WNA)
...
![](https://waytoagi.feishu.cn/space/api/box/stream/download/asynccode/?code=OGMxYzE3MjA4NGE2Njg2YmRmNjMyMjdiZTZkNTY3ZGNfUnV0VmRYdFVVOG9YcTI5b3RSbGJTUDh4bTJtZDFXSkFfVG9rZW46QWRIdmJXWE5LbzVTMDl4OEZmTWN2M3hMbmhjXzE3NzkyODY2NTQ6MTc3OTI5MDI1NF9WNA)
...
![](https://waytoagi.feishu.cn/space/api/box/stream/download/asynccode/?code=ZTlmNTg2MTZmNzdiNzg2M2E1NmU0YTFhMDg2OTMxYjRfcTZaUnNSbmFpOHJ6eFc4SHEyOUxSbkFZb1FlQnNjcTZfVG9rZW46UEM2R2JOSjhGb3BkSDJ4eDd2S2NoS0JTblRjXzE3NzkyODY2NTQ6MTc3OTI5MDI1NF9WNA)
...
![](https://waytoagi.feishu.cn/space/api/box/stream/download/asynccode/?code=OTVlOTIyNTQ5NzgxZmUyYWMyOGI0MmM4YmE2ODA4NmZfN0E2MzdVOVBDM2Q0NlgzbXQxMm43RmMzdkhkY0I3Z0xfVG9rZW46RlBSbGIzeDRkb3RyU2J4VTd2TmMxdWlybkZmXzE3NzkyODY2ODg6MTc3OTI5MDI4OF9WNA)
...
![](https://waytoagi.feishu.cn/space/api/box/stream/download/asynccode/?code=NGRkN2VhNTg3NzFmNjVmNWNhYTkxYjJhZTcwNjdmYTZfSm1LU1BudWRqb3lhWXZiVXllU1U4U1l5T3lmalJZZjJfVG9rZW46T0l2bmJnWDUyb1lTVjJ4eW5sTmNiSU1abkJoXzE3NzkyODY2ODg6MTc3OTI5MDI4OF9WNA)
...
def get_weather(city: str) -> str:"""
    ...
    """# API端点，我们请求JSON格式的数据
    url = f"https://wttr.in/{city}?format=j1"try:# 发起网络请求
        response = requests.get(url)# 检查响应状态码是否为200 (成功)
        response.raise_for_status() 
        # 解析返回的JSON数据
        data = response.json()# 提取当前天气状况
        current_condition = data['current_condition'][0]
        weather_desc = current_condition['weatherDesc'][0]['value']
        temp_c = current_condition['temp_C']# 格式化成自然语言返回return f"{city}当前天气:{weather_desc}，气温{temp_c}摄氏度"except requests.exceptions.RequestException as e:# 处理网络错误return f"错误:查询天气时遇到网络问题 - {e}"except (KeyError, IndexError) as e:# 处理数据解析错误return f"错误:解析天气数据失败，可能是城市名称无效 - {e}"
```
...
def get_attraction(city: str, weather: str) -> str:"""
    ...
    """# 1. 从环境变量中读取API密钥
    api_key = os.environ.get("TAVILY_API_KEY")if not api_key:return "错误:未配置TAVILY_API_KEY环境变量。"# 2. 初始化Tavily客户端
    tavily = TavilyClient(api_key=api_key)# 3. 构造一个精确的查询
    query = f"'{city}' 在'{weather}'天气下最值得去的旅游景点推荐及理由"try:# 4. 调用API，include_answer=True会返回一个综合性的回答
        response = tavily.search(query=query, search_depth="basic", include_answer=True)# 5. Tavily返回的结果已经非常干净，可以直接使用# response['answer'] 是一个基于所有搜索结果的总结性回答if response.get("answer"):return response["answer"]# 如果没有综合性回答，则格式化原始结果
        formatted_results = []for result in response.get("results", []):
            formatted_results.append(f"- {result['title']}: {result['content']}")if not formatted_results:return "抱歉，没有找到相关的旅游景点推荐。"return "根据搜索，为您找到以下信息:\n" + "\n".join(formatted_results)except Exception as e:return f"错误:执行Tavily搜索时出现问题 - {e}"
```
...
class OpenAICompatibleClient:"""
    ...
    """def __init__(self, model: str, api_key: str, base_url: str):
        self.model = model
        self.client = OpenAI(api_key=api_key, base_url=base_url)def generate(self, prompt: str, system_prompt: str) -> str:"""调用LLM API来生成回应。"""print("正在调用大语言模型...")try:
            ...
            print("大语言模型响应成功。")return answer
        except Exception as e:print(f"调用LLM API时发生错误: {e}")return "错误:调用语言模型服务时出错。"
```
...
# --- 1. 配置LLM客户端 ---# 请根据您使用的服务，将这里替换成对应的凭证和地址
...
prompt_history = [f"用户请求: {user_prompt}"]print(f"用户输入: {user_prompt}\n" + "="*40)# --- 3. 运行主循环 ---for i in range(5): # 设置最大循环次数print(f"--- 循环 {i+1} ---\n")# 3.1. 构建Prompt
    full_prompt = "\n".join(prompt_history)# 3.2. 调用LLM进行思考
    llm_output = llm.generate(full_prompt, system_prompt=AGENT_SYSTEM_PROMPT)# 模型可能会输出多余的Thought-Action，需要截断match = re.search(r'(Thought:.*?Action:.*?)(?=\n\s*(?:Thought:|Action:|Observation:)|\Z)', llm_output, re.DOTALL)if match:
        truncated = match.group(1).strip()if truncated != llm_output.strip():
            llm_output = truncated
            print("已截断多余的 Thought-Action 对")print(f"模型输出:\n{llm_output}\n")
    prompt_history.append(llm_output)# 3.3. 解析并执行行动
    action_match = re.search(r"Action: (.*)", llm_output, re.DOTALL)if not action_match:print("解析错误:模型输出中未找到 Action。")break
    action_str = action_match.group(1).strip()if action_str.startswith("finish"):
        final_answer = re.search(r'finish\(answer="(.*)"\)', action_str).group(1)print(f"任务完成，最终答案: {final_answer}")break
    
    tool_name = re.search(r"(\w+)\(", action_str).group(1)
    args_str = re.search(r"\((.*)\)", action_str).group(1)
    kwargs = dict(re.findall(r'(\w+)="([^"]*)"', args_str))if tool_name in available_tools:
        observation = available_tools[tool_name](**kwargs)else:
        observation = f"错误:未定义的工具 '{tool_name}'"# 3.4. 记录观察结果
...
![](https://waytoagi.feishu.cn/space/api/box/stream/download/asynccode/?code=YmFkN2EyY2M1MjZiY2ZiNzE5MzAxMjg2NjgyYWQ2MThfNzFNT0dCbTJwdUh4U1U4TGkwV1lteHlSN1RNcHZNT09fVG9rZW46SjE0SmI0WDhZb1NBakJ4c3lJVmMyZk5UbjRmXzE3NzkyODY3MjU6MTc3OTI5MDMyNV9WNA)
...
2. `case A`：一台符合冯·诺依曼结构的超级计算机，拥有高达每秒 2EFlop 的峰值算力
    
3. `case B`：特斯拉自动驾驶系统在高速公路上行驶时，突然检测到前方有障碍物，需要在毫秒级做出刹车或变道决策
    
4. `case C`：AlphaGo在与人类棋手对弈时，需要评估当前局面并规划未来数十步的最优策略
    
5. `case D`：ChatGPT 扮演的智能客服在处理用户投诉时，需要查询订单信息、分析问题原因、提供解决方案并安抚用户情绪
    
6. 假设你需要为一个"智能健身教练"设计任务环境。这个智能体能够：
    
    1. 通过可穿戴设备监测用户的心率、运动强度等生理数据
        
    2. 根据用户的健身目标（减脂/增肌/提升耐力）动态调整训练计划
        
    3. 在用户运动过程中提供实时语音指导和动作纠正
        
    4. 评估训练效果并给出饮食建议
        
7. 请使用 PEAS 模型完整描述这个智能体的任务环境，并分析该环境具有哪些特性（如部分可观察、随机性、动态性等）。
    
8. 某电商公司正在考虑两种方案来处理售后退款申请：
    
9. 方案 A（`Workflow`）：设计一套固定流程，例如：
    
10. A.1 对于一般商品且在 7 天之内，金额 `< 100RMB` 自动通过；`100-500RMB` 由客服审核；`>500RMB` 需主管审批；而特殊商品（如定制品）一律拒绝退款
    
11. A.2 对于超过 7 天的商品，无论金额，只能由客服审核或主管审批；
    
12. 方案 B（`Agent`）：搭建一个智能体系统，让它理解退款政策、分析用户历史行为、评估商品状况，并自主决策是否批准退款
...
14. 在 1.3 节的智能旅行助手基础上，请思考如何添加以下功能（可以只描述设计思路，也可以进一步尝试代码实现）：
    

> 1. 提示：思考如何修改 `Thought-Action-Observation` 循环来实现这些功能。
>     
...
15. 卡尼曼的"系统 1"（快速直觉）和"系统 2"（慢速推理）理论([2])为神经符号主义 AI 提供了很好的类比。请首先构思一个具体的智能体的落地应用场景，然后说明场景中的：
    

> 2. 提示：医疗诊断助手、法律咨询机器人、金融风控系统等都是常见的应用场景
>     
>     1. 哪些任务应该由"系统 1"处理？
>         
>     2. 哪些任务应该由"系统 2"处理？
...
16. 尽管大语言模型驱动的智能体系统展现出了强大的能力，但它们仍然存在诸多局限。请分析以下问题：
